...
namespace PMT.DAL
{
	...
	public partial class Task : ActiveRecord<Task>, IActiveRecord
	{
		...
		public PMT.DAL.UserCollection GetUserCollection() { return Task.GetUserCollection(this.Id); }
		public static PMT.DAL.UserCollection GetUserCollection(int varId)
		{
		    SubSonic.QueryCommand cmd = new SubSonic.QueryCommand("SELECT * FROM [dbo].[Users] INNER JOIN [TaskAssignments] ON [Users].[ID] = [TaskAssignments].[TaskID] WHERE [TaskAssignments].[TaskID] = @TaskID", Task.Schema.Provider.Name);
		...
		public static void SaveUserMap(int varId, UserCollection items)
		{
			...
			QueryCommand cmdDel = new QueryCommand("DELETE FROM [TaskAssignments] WHERE [TaskAssignments].[TaskID] = @TaskID", Task.Schema.Provider.Name);
